...
class ClassificationAttacker(Attacker):
    ...
    def __call__(self, victim: Classifier, input_: Any):
        ...
        if "target" in input_:
            ...
            origin_x = victim.get_pred([ input_["x"] ])
            if 'negative' in origin_x:
                origin_x_num = 0
            elif 'negati' in origin_x:
                origin_x_num = 0
            elif 'nega' in origin_x:
                origin_x_num = 0
            elif 'neg' in origin_x:
                origin_x_num = 0
            elif 'negat' in origin_x:
                origin_x_num = 0
            elif 'ne' in origin_x:
                origin_x_num = 0
            else:
                origin_x_num = 1
            goal = ClassifierGoal( origin_x_num, targeted=False )
