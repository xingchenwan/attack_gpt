...
class DataManager(object):
    ...
    It reads configuration files in TextAttack/data/\*.py, and initialize these resources when you load them.

    You can use 

    .. code-block:: python
    
        for data_name in TextAttack.DataManager.AVAILABLE_DATAS:
            TextAttack.download(data_name)
     
    to download all the available resources, but this is not recommend because of the huge network cost.

    ``TextAttack.load`` and ``TextAttack.download`` is a alias of 
    ``TextAttack.DataManager.load`` and ``TextAttack.DataManager.download``, they are exactly equivalent.
    ...
    data_path = {
        x["name"]: os.path.join(os.getcwd(), "data", x["name"]) for x in data_list # text_classification/
    }
