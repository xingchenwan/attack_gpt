...
def get_language_tool_download_path(cache_path):
    # Get download path from environment or use default.
    download_path = os.environ.get(
        'LTP_PATH',
        os.path.join(cache_path, "language_tool_data")
    )
    # Make download path, if it doesn't exist.
    os.makedirs(download_path, exist_ok=True)
    return download_path

def get_language_tool_directory(cache_path):
    """Get LanguageTool directory."""
    download_folder = get_language_tool_download_path(cache_path)
...
def get_server_cmd(cache_path, port: int=None, config: LanguageToolConfig=None) -> List[str]:
    java_path, jar_path = get_jar_info(cache_path)
...
def get_jar_info(cache_path) -> Tuple[str, str]:
    java_path = which('java')
    if not java_path:
        raise JavaError("can't find Java")
    dir_name = get_language_tool_directory(cache_path)
    jar_path = None
